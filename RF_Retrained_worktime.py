...
This code has been modified to generate an optimized Random Forest proxy model
for simulating the time required for a PEMFCs to warm up from 30℃ to 70℃ during cold-start.
...
if __name__ == "__main__":
    ...
    search_space = [
        ...
        Integer(2, 30, name='max_depth'),  # 树的最大深度
        Integer(2, 10, name='min_samples_split'),  # 分裂内部节点所需的最小样本数
        Integer(1, 10, name='min_samples_leaf'),  # 叶节点所需的最小样本数
        Categorical(['sqrt', 'log2'], name='max_features')  # 寻找最佳分割时要考虑的特征数量
    ]
    ...
    # 输出最优参数
    print("\n最优RF参数:")
    print(f"树的数量: {result.x[0]}")
    print(f"树的最大深度: {result.x[1] if result.x[1] != -1 else 'None'}")
    print(f"分裂内部节点所需的最小样本数: {result.x[2]}")
    print(f"叶节点所需的最小样本数: {result.x[3]}")
    print(f"寻找最佳分割时要考虑的特征数量: {result.x[4]}")
    ...
    # 保存模型
    joblib.dump(model, 'worktime_RF_optimized.pkl')
    print(f"优化后的RF模型已保存: worktime_RF_optimized.pkl")
    ...
    def plot_optimization_results(result):
        ...
        plt.title('Bayesian Optimization of RF Parameters', fontsize=14)
        plt.grid(True, linestyle='--', alpha=0.5)
        plt.tight_layout()
