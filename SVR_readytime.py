...
This code has been modified to generate an optimized SVR model with refined
Bayesian hyperparameter tuning for simulating equired for a PEMFCs to warm up from -20℃ to 30℃ during cold-start.
...
def objective(params):
    ...
    """
    kernel_type, C, gamma, epsilon = params

    # 初始化SVR模型（移除random_state参数）
    model = SVR(
        kernel=kernel_type,
        C=C,
        gamma=gamma,
...
if __name__ == "__main__":
    # 数据加载与处理
    print("Loading data...")
    data = pd.read_csv("TRAIN_readytime.csv")
    data = np.array(data)
    ...
    # ================== 贝叶斯优化SVR超参数 ==================
    print("\n开始贝叶斯优化SVR超参数（优化搜索边界）...")

    # 定义SVR的搜索空间
    search_space = [
        Categorical(['rbf', 'linear', 'poly'], name='kernel_type'),  # 核函数类型
        Real(1e-2, 1e2, name='C'),  # 正则化参数
        Real(1e-4, 10, name='gamma'),  # 核函数系数
        Real(1e-4, 1e-1, name='epsilon')  # 容忍误差
    ]
    ...
    # 输出最优参数
    print("\n最优SVR参数:")
    print(f"核函数: {result.x[0]}")
    print(f"正则化参数C: {result.x[1]:.4f}")
    print(f"核系数gamma: {result.x[2]:.6f}")
    print(f"容忍误差epsilon: {result.x[3]:.6f}")
    print(f"交叉验证RMSE: {result.fun:.6f}")

    # ================== 使用最优参数训练最终模型 ==================
    print("\n使用最优参数训练最终SVR模型...")

    # 构建最优模型（移除random_state参数）
    best_model = SVR(
        kernel=result.x[0],
        C=result.x[1],
        gamma=result.x[2],
    ...
    # 保存模型
    joblib.dump(best_model, 'readytime_SVR_optimized.pkl')
    print(f"优化后的SVR模型已保存: readytime_SVR_optimized.pkl")
    ...
    # ================== SVR特有可视化：支持向量分布 ==================
    def plot_support_vectors(model, x_data, y_data):
        """可视化支持向量分布"""
        # 仅当核函数为RBF或线性时有效
        if model.kernel in ['rbf', 'linear']:
            plt.figure(figsize=(10, 6))

            # 反归一化数据
            x_denorm = denormalize(x_data, norms)
            y_denorm = denormalize(y_data, norms, is_output=True).flatten()

            # 绘制所有样本
            plt.scatter(x_denorm[:, 0], y_denorm, c='#cccccc', alpha=0.3, s=30, label='All Samples')

            # 绘制支持向量
            sv_indices = model.support_
            plt.scatter(x_denorm[sv_indices, 0], y_denorm[sv_indices], c='#ff2501', alpha=0.8, s=60,
                        label='Support Vectors')

            plt.xlabel('Feature 1 (Denormalized)', fontsize=12)
            plt.ylabel('Ice Volume Fraction', fontsize=12)
            plt.title('Support Vectors Distribution', fontsize=14)
            plt.legend()
            plt.grid(True, linestyle='--', alpha=0.5)
            plt.tight_layout()
            plt.savefig('support_vectors.png', dpi=300)
            plt.show()
        else:
            print("Support vector visualization not supported for polynomial kernel.")
    ...
    def plot_optimization_results(result):
        ...
        plt.title('Bayesian Optimization of SVR Parameters', fontsize=14)
        plt.grid(True, linestyle='--', alpha=0.5)
        plt.tight_layout()
    ...
    # 新增：支持向量可视化（仅当特征数≤2时有效）
    if x.shape[1] <= 2:
        plot_support_vectors(best_model, x, y)
    else:
        print("Support vector visualization requires ≤2 features.")
